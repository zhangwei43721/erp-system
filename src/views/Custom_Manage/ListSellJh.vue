<template>
  ...
  <el-table :data="sellJHList" stripe style="width: 100%">
    <el-table-column label="计划编号" prop="id" width="180" />
    <el-table-column label="客户" prop="custName" width="180" />
    <el-table-column label="销售渠道" prop="channelId">
      <template #default="scope">
        {{ getChannelName(scope.row.channelId) }}
      </template>
    </el-table-column>
    <el-table-column label="销售金额" prop="money" />
    <el-table-column label="销售阶段" prop="nowStep" />
    <el-table-column label="业务员" prop="empId" />
    <el-table-column fixed="right" label="操作" width="120">
      <template #default="scope">
        <el-button link size="small" type="primary" @click="deleteSellJh(scope.row.id)">删除</el-button>
        <el-button link size="small" type="primary" @click="openSellJhDialog(scope.row)">修改</el-button>
      </template>
    </el-table-column>
  </el-table>
  ...
  <el-pagination :page-size="10" :pager-count="5" :total="total" background class="mt-4" layout="prev, pager, next"
    size="small" @current-change="handlerPageChange" />

  <el-dialog v-model="dialogSellJhVisible" width="80%">
    <h2>修改销售计划</h2>
    <el-form ref="sellJhFormRef" :model="sellJhForm" :rules="rules" label-width="120px">
      <el-form-item label="客户名称">
        <el-select v-model="sellJhForm.custid" class="m-2" placeholder="请选择客户" size="large" style="width: 80%">
      ...
      <el-form-item label="销售渠道" prop="channelId">
        <el-select v-model="sellJhForm.channelId" placeholder="请选择渠道...." style="width: 80%">
          <el-option v-for="opt in channelList" :key="opt.id" :label="opt.label" :value="opt.id" />
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="业务员" prop="empId">
        <el-input v-model.number="sellJhForm.empId" readonly="readonly" style="width: 80%" />
      </el-form-item>
      <el-form-item>
