<template> <!-- 1. 模板区域: 定义组件的HTML结构 -->
  ...
  <el-table :data="custList" stripe style="width: 100%">
    ...
    <el-table-column label="客户职业" prop="custType"/>
    <el-table-column label="客户等级" prop="grade"/>
    <el-table-column label="消费总额（元）" prop="hisTotal"/>

    <!-- 操作列: fixed固定在右侧, width设置宽度, #default作用域插槽自定义内容 (row代表当前行数据) -->
    <el-table-column fixed="right" label="操作" width="120">
      <template #default="{ row }">
        <!-- 饿了么按钮: link类型像链接, 小按钮样式，@click绑定点击事件 -->
        <el-button link size="small" type="primary" @click="deleteCustomer(row.id)">删除</el-button>
        <el-button link size="small" type="primary" @click="openCustDialog(row)">修改</el-button>
      </template>
    </el-table-column>
  </el-table>
  <hr/>
  ...
  <el-dialog v-model="dialogCustVisible" title="修改客户信息" width="80%">
    <!-- 客户信息表单 (饿了么Plus组件 el-form) -->
    <!-- :model绑定表单数据对象, label-width标签宽度, :rules绑定验证规则, ref用于访问表单实例 -->
    <el-form ref="custFormRef" :model="custForm" :rules="rules" label-width="120px">
